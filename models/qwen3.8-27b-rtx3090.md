...
### 2026-09-11: Operating-envelope campaign — knee confirmed at k=3, card is freeable
- Overnight v2 campaign ([report](../reports/2026-09-11-dual3090-v2-operating-envelope.md)): MTP envelope on the 8192 profile — k=0/2/3/4 = 67.1/113.4/151.1/167.6 t/s @16K; **knee at k=3** (k4 only +11%), so production stays at k=3. Single-stream 16K on the live endpoint: 147.8 t/s (IQR 1.1). **Prefill interference measured: a 64K prefill in flight raises running-decode ITL 25.8 → 39.2 ms (+51.9%)** on the 8192 profile, with full recovery after — the key input for the 8192-vs-2048 batch-budget decision (48 h passive monitoring verdict due same day). **TP1 (single 3090) = 149.8 t/s ≈ TP2 147.8** at single-user load → **one 3090 is freeable for a second service**. 32K prefix cache: 36.5 s cold → 1.22 s on a spaced re-send (~30×), timing-sensitive. Concurrency rows invalid by design (512-out vs 30 s overlap gate) — cells redesigned for the next window. First unattended campaign after the two 09-08/09-10 crashes: the box held, recovery stack unused.
