...
**Eviction telemetry (staged, off):** vLLM 0.28.0 exposes the prefix-cache eviction histograms (`vllm:kv_block_lifetime_seconds`, `..._idle_before_evict_seconds`, `..._reuse_gap_seconds`) behind the startup flags `--kv-cache-metrics` (+ `--kv-cache-metrics-sample`, default 0.01) — no runtime toggle, so enabling needs one cold restart. A staged, idempotent script in the launch dir does the edit + restart + verify with a documented rollback; it has not been applied (dry-run verified 2026-09-16) — apply in a maintenance window, then the evictions become measurable instead of silent LRU.
