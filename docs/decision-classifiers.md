...
| **Abstract-comparative** — the answer requires comparing two descriptions against each other ("do claim A and claim B contradict?") | the [doc-staleness detector](#the-doc-staleness-application) | noul **inverts** (answers the implicit "do these match?" — measured in both polarities across two wordings, so a type problem, not wording); 2-way is *worse* than 3-way (the escape option helps the model commit); an explicit subject prefix fixes labels but collapses confidence; and **confidence cannot gate** — correct answers arrive at 1–65%. The reframe that restores the concrete regime: make the comparison *observable* (NLI on concrete value mismatches, not on vibes). **Measured on a 160-pair mined corpus (2026-09-22):** a ~0.4B *SFT-trained* NLI transformer (ModernBERT-large-nli, MNLI 0.89) in that reframe — 421M question model vs it: the NLI model flips the failure axis (recall 0.84–0.92 at gate 0.5 vs 0.05–0.08, but precision@confident 0.26–0.39, i.e. closed-world over-firing on cross-document technical statements: a claim that *mentions more* reads as contradicting a fact that mentions *less*); its confidence is roughly monotone (the 421M's was not). Standalone it fails a precision-first contract; **inside the cascade it is the better first tier** — with the 27B as arbiter of record it cut arbiter calls ~26% and cleared 3–5× the boring pairs while holding final precision at 1.0 and recall ≥ 0.9 on both splits (dev 117 / heldout 43, split by document). Its characteristic false alarm (two *genuinely co-stale* statements) is exactly what the arbiter veto exists to absorb. |
...
2. **An SFT NLI fine-tune of the same backbone** — [tasksource/ModernBERT-large-nli](https://huggingface.co/tasksource/ModernBERT-large-nli) (0.4B; MNLI 0.89, "better than llama 3.1 8B Instruct on ANLI and FOLIO"). Standard supervised head → routine LoRA, standard softmax → temperature scaling applies directly. **Zero-shot test done (2026-09-22, see the regime map):** it does not beat the 421M *standalone* (precision@confident 0.26–0.39, closed-world over-firing), but it is the better **cascade first tier** (−26% arbiter calls, 3–5× local clearance, precision 1.0 / recall ≥ 0.9 with the 27B arbiter). The remaining gap a fine-tune could close: the ~10% of true contradictions it confidently clears (the "mentions-more ≠ contradiction" cases) plus its noise verdicts — training on the adjudicated mined corpus is the candidate that targets exactly that.
