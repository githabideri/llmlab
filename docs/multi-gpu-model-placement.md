# Multi-GPU Model Placement in llama.cpp

> **Observed results** are measurements from specific hardware/builds. **Guidance** is the current recommendation derived from them and may change with llama.cpp behavior. Placement-related behavior (fitter, tensor overrides, MTP, lazy loading, split support) is **commit-sensitive** — exact build provenance is mandatory.

**Scope:** Where model tensors *live and execute* across multiple GPUs (and CPU) on consumer PCIe-only systems: automatic fitter placement, manual layer/tensor split, CPU/expert spill, heterogeneous VRAM balancing, and the validation that proves what actually crosses PCIe. Measurement discipline lives in [benchmarks](benchmarks.md); memory budgeting in [kv-cache-sizing](kv-cache-sizing.md). This page started in March 2026 as a `--tensor-split` tuning guide; 2026-08 (tensor mode) and 2026-09 (the fitter) widened its scope, hence the rename.

**Target:** Consumer multi-GPU systems without NVLink (PCIe only)

## Placement decision tree

> **Guidance (2026-09).** On current llama.cpp builds with the fitter enabled, **try automatic placement before manual `-ngl`/`--tensor-split` tuning** whenever the model is capacity-constrained or contains tensors with very different placement characteristics. Benchmark it *first*; override it only for a demonstrated reason.

```
current llama.cpp build?
├─ yes (fitter present)
│    1. try AUTO: no --gpu-layers / --tensor-split / --override-tensor
│    2. fitter places a valid config? → benchmark it FIRST; keep if it meets target
├─ fitter aborts, or you need a frozen placement, or the architecture benefits
│  from explicit tensor parallelism (homogeneous cards, fully resident)
│    3. → manual placement below: -ngl / --split-mode / --tensor-split / --override-tensor
└─ either path
     4. CPU/expert spill only when VRAM genuinely doesn't close
     5. never trust "it fits" — prove residency with PCIe counters + the startup allocation log
```

Rules of thumb accumulated so far — all **Observed** in this fleet, not universal laws:

| Situation | First candidate |
|---|---|
| Capacity-constrained mix, or model with giant host-side tensors (e.g. Qwen4Exp's 51 B PLE) — current build | **auto fitter** (2026-09: beat hand-tuned layer placement, 21 → 30 t/s on 12+12+24 GB) |
...
## Manual placement: `--tensor-split` and friends
...
**Start with `--split-mode layer`** — unless the fully-resident + equal-card case below applies (then tensor).
...
## Automatic fitter placement (2026-09)

The Qwen4Exp campaign ([report](../reports/2026-09-02-qwen4exp-flash-next-three-gpu-campaign.md)) added the strategy that tops the decision tree: **setting no placement flags at all.**

> **Current build behavior (master `b81c99b4`, observed 2026-09).** Launching with no `--gpu-layers`, no `--tensor-split`, no `--override-tensor` invokes `common_fit_params`, which byte-aware-places every tensor across the available cards *and* selectively spills individual weight tensors (here: a few layers' expert weights) to host RAM. The fitter's contract is to modify only parameters that are still at their defaults — which is exactly why any explicit placement flag switches it off.

**Observed (2026-09, 3060/3060/3090, Qwen4Exp Q2_K_XL):** residency 11.2 / 10.7 / 23.0 GiB; **33.4 t/s** exploratory, **30.2 t/s** repeat-median decode — vs **21.2 t/s** median for the best hand-tuned whole-layer config (ngl 44, `--tensor-split 1,1,2`); prefill roughly 2× under comparable cache states.

**Why selective spill beats whole-layer spill here:** a manual ngl boundary drags ~0.9 GB of experts to RAM *per* spilled layer and re-reads them every decode step; the fitter keeps all 48 layers' small non-expert weights + shared experts resident and sheds only the expert mass needed to close the VRAM gap.

Hard rules — **Current build behavior (master)**, each hit in the campaign:

1. **Any explicit `--gpu-layers` or `--tensor-split` disables the fitter** ("failed to fit params: tensor_split already set by user"). Count-based hand-balancing (`--cpu-moe`, `--n-cpu-moe N`) then demanded 12.9 GB from a 12 GB card, identically, on every variant.
...
3. `--fit-target <MiB>` (default 1024) tunes the fitter's per-device free-VRAM margin; 512–3072 barely changed placement.

> **Runtime/version sensitivity.** Fitter behavior, `--override-tensor` parsing, lazy-PLE direct reads (PR #28136), MTP support, and even the ability to *load* a model are commit-sensitive — this box cannot load the Qwen4Exp GGUF on its production build at all. Record the exact commit in every auto-placement campaign and re-verify placement after any llama.cpp update.
...
For a new model on a multi-GPU box:

- [ ] **Try auto placement first** (current build, no placement flags) — the fitter spills individual tensors, not whole layers, and can beat manual ngl/split; benchmark before deciding
- [ ] Override the fitter only if it aborts, misplaces, or you need a frozen placement
- [ ] Manual path: start even; find the bottleneck GPU in `nvidia-smi`; account for output.weight on the last GPU (~1–2 GB); adjust ratios in 0.03–0.05 steps; verify with actual context fill
- [ ] `--split-mode layer` for PCIe (tensor only for the fully-resident + equal-card case); never `row`
...
- [ ] `--parallel N` trades concurrency for capacity
- [ ] Record per device: UUID, PCI bus ID, model, CUDA ordinal, negotiated link width ([benchmarks](benchmarks.md))

---

## References

- **llama.cpp source:** `llama-model.cpp:2762` (output.weight placement); `common/fit.h` (fitter contract: modifies only parameters still at their defaults)
...
- **Case studies:** [2026-09-02 Qwen4Exp three-GPU campaign](../reports/2026-09-02-qwen4exp-flash-next-three-gpu-campaign.md) (fitter vs manual) · [2026-08-27 dual-3060 optimization](../reports/2026-08-27-qwen3.6-35b-a3b-dual-3060-optimization.md) (tensor mode beats layer when fully resident)
