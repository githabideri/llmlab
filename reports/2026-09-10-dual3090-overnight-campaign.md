...
>
> **Clarification (2026-09-13):** the ~23 GB/card `nvidia-smi` reading seen around the B1 re-test was **not** the vLLM production workload and does **not** correspond to allocatable VRAM. The main GPU server's two 3090s are shared by the vLLM node and the llama.cpp test node (no GPU partitioning); that ~23 GB is a host-side accounting reading that does not constrain allocations — the same reading was present during the later single-3090 run [2026-09-13](2026-09-13-flash-next-single3090-codacus-cache.md) while ~19 GB of CUDA allocations still succeeded. B1 is therefore best read as an **unsplit ~25.4 GB allocation failing on a single 24 GB card** in the experimental tensor path, not as contamination from the production service. The September 10 wall is unaffected.
