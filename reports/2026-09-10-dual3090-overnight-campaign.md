...
> **Update (2026-09-12):** a re-test with the experimental Qwen4Exp tensor-split build (llama.cpp [#28569](https://github.com/ggml-org/llama.cpp/pull/28569): re-enables `tensor` mode + a meta-scheduler placement fix) was **inconclusive** — it died at load on a meta-backend `GGML_ASSERT` and left a **~25.4 GB buffer unsplit on a single device** (a build/runtime failure, **not** the per-layer 25.75 GB wall above — different mechanism, different number). The mainline control re-confirms `tensor`/`row` unsupported and `layer` OOM. So **the September 10 load wall remains the last valid result for the tested build family**; the 09-12 run adds no clean confirmation, and C/D/E were gated by this failed validation, not by a proven hardware limit.
>
> **This does *not* establish that the model is too large for 2×24 GB.** The Q2 artifact's **non-PLE weights are 45.9 GB** — inside the pair's ~48.6 GB aggregate VRAM — while the **51 B PLE table stays host/mmap/SSD-backed** (the 78.9 GB GGUF figure counts that host-resident PLE, which is *not* part of the VRAM budget; see the [model card](../models/qwen3.8-flash-next.md) and [2026-09-02](2026-09-02-qwen4exp-flash-next-three-gpu-campaign.md)). A correctly-functioning CUDA tensor-split path shreds each tensor's rows across both GPUs (unlike `layer` mode, which stacked a 25.75 GB whole-layer buffer on one card), so it **could still make dual-3090 execution viable**. The decisive open question is whether a working current CUDA tensor path can actually shard the Qwen4Exp tensors; the next step is **diagnosis, not re-campaigning** — confirm the exact command + binary, force PLE→CPU/mmap, minimal ctx, and capture the full tensor-allocation stderr to see whether #28569 needs another CUDA patch.
>
> **Clarification (2026-09-13):** the ~23 GB/card `nvidia-smi` reading seen around the B1 re-test was **not** the vLLM production workload. The main GPU server's two 3090s are shared by the vLLM node and the llama.cpp test node (no GPU partitioning), and that ~23 GB is a **persistent, non-binding reading observed independently of successful CUDA allocations** — the same reading was present during the later single-3090 run [2026-09-13](2026-09-13-flash-next-single3090-codacus-cache.md) while ~19 GB of CUDA allocations still succeeded. B1 is therefore best read as a **~25.4 GB allocation that remained on one CUDA device instead of being distributed across the pair** in the experimental tensor path — i.e. #28569 did not produce the intended usable 1:1 pattern in that run — and **not** as contamination from the production service. The September 10 wall is unaffected.
