...
- **RAM is a near-edge constraint, and it is a knob not a wall:** the llama.cpp node was allocated **~48 GiB** with **no hard cgroup memory cap** and **no swap** — below the 64 GB the brief assumed. The run's usage, measured as a **host-observed, cache-inclusive** reading, reached **~48.7 GiB without any OOM**. (The 48.7 and the 48 are *different measurements* — a host-level, cache-inclusive usage counter vs. the LXC allocation — not usage exceeding a ceiling; with no hard cap and no swap there was no OOM.) The host has headroom to raise the allocation toward **60 GiB** (a 60 GiB allocation was already used in the 09-12 window). So host RAM — not just VRAM — is a real, near-edge constraint here, which argues for tuning cache/profile memory against measured host pressure and, if the profile grows, raising the allocation toward 60 GiB in future runs.
- **`nvidia-smi` caveat:** on this node the `nvidia-smi` used-count shows a **persistent ~23 GB/card reading that did not track effective CUDA allocation headroom** — it was present while `cudaMalloc` still allocated the 4.8 GB baseline and the ~19 GB cache pack successfully. Whatever its source, it is a **non-binding** reading here, so the VRAM figures below come from the CUDA allocator and the fork's own fit reporting, not the raw `nvidia-smi` used-count.
...
**Measurement protocol:** streaming `/v1/completions`, temperature 0, seed 42, `stream:true`. Decode t/s = (SSE content-chunk count − 1) / (wall − TTFT); prefill t/s = prompt tokens / TTFT. **Token basis:** this fork's `/v1/completions` emits one stream chunk per generated token, and the client counts those content chunks (the `[DONE]` terminator is excluded), so under that contract chunk/s equals token/s. **This run did not request `stream_options.include_usage` or otherwise cross-check the chunk count against a server-side token counter** — the rate is valid under llama.cpp's one-token-per-chunk streaming and should be reconfirmed against server token counters before being treated as an exact reference. One warm-up rep + 3 measured per prompt, reported as **medians over reps 1–3** (rep 0 is the cold, no-prompt-cache prefill). Two prompt families: **P1** (short prose, natural EOS at 264–288 tokens) and **P2** (code/reasoning review, 1025-token prompt, EOS-capped).
...
- **09-12 B1** — the #28569 tensor re-test was **inconclusive**: a ~25.4 GB allocation remained on one CUDA device instead of being distributed across the pair (i.e. the intended 1:1 pattern did not materialize in that run). (A later clarification established the ~23 GB `nvidia-smi` reading was *not* production-vLLM contamination; see the companion report's note.)
...
- **Measured:** S0/S1a decode and prefill rates, per-rep values, GPU VRAM, per-slot 85.76 MiB, the 192/200/208/224/256 capacity ladder, within-config determinism, S0≠S1a divergence from the first token, output coherence, host-level (cache-inclusive) RAM usage ~48.7 GiB with no OOM, single-GPU usage (2D active / 2E idle), vLLM stop/restore, restoration canaries.
